...
### warping
if [ $1 == 1 ]; then
    python -m torch.distributed.launch --nproc_per_node=8 --master_port=4739 test_warping.py \
...
### for gen
    ...
    python -m torch.distributed.launch --nproc_per_node=8 --master_port=4736 test_tryon.py \
        ...
        --dataroot /dataset/VITON-HD \
        --image_pairs_txt test_pairs_unpaired_1018.txt \
        --warproot /dataset/GP-VTON-partflow_final_kaiwu/sample/test_partflow_vitonhd_unpaired_1109
fi
